...
export class GstInvoiceComponent implements OnInit {
  ...
  updateProductDetails(index: number) {
    ...
    const product = this.products.find(p => p._id === itemGroup.value.product);
    if (product) {
  ...
  calculateItemTotal(index: number) {
    ...
    const discountedValue = taxableValue - discountAmount;
    const gstAmount = discountedValue * (item.gstRate / 100);
    const amount = discountedValue + gstAmount;
  ...
  calculateTotals() {
    ...
    const totalDiscount = items.reduce((sum: number, item: any) => sum + (item.taxableValue * item.discount / 100), 0);
    const gst = items.reduce((sum: number, item: any) => sum + item.gstAmount, 0);
    const totalAmount = subTotal + gst - totalDiscount;
  ...
  calculateItemAmount(index: number): number {
    return this.items.at(index).value.amount;
  }

  async loadData() {
    try {
      this.sellers = await this.invoiceService.getSellers().toPromise();
      this.customers = await this.invoiceService.getCustomers().toPromise();
      this.products = await this.invoiceService.getProducts().toPromise();
    } catch (error) {
